...
def init():
	top_bar_plugin_manager = tk.Menu(
			application.top_bar,
			tearoff = 0,
	...
	top_bar_plugin_manager.add_command(
			label = "Manager Plugins",
			command = open_manager_window,
			font = application.top_bar_font
	)

	application.top_bar.add_cascade(

			label = "Plugins",
			menu = top_bar_plugin_manager,
			font = application.top_bar_font
	)
	pass
	...
	def __init__(self, master: tk.Tk):
		...
		for n, kv in enumerate(plugins.items()):
			...
			b = tk.Button(
					...
					command = lambda: self.on_press(k, n),
					bg = COLOR_TEXT_STR if v["is_enabled"] else COLOR_ERROR
			)
